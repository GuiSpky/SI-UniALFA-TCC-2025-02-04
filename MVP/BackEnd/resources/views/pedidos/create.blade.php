    ...
    <div class="container-fluid py-4">
        <div class="mb-4 fade-in-up">
            <div class="d-flex align-items-center mb-3">
                <a href="{{ route('pedidos.index') }}" class="btn btn-outline-secondary btn-sm me-3">
                    <i class="bi bi-arrow-left me-2"></i>Voltar
                </a>
                <div>
                    <h1 class="h2 fw-bold mb-1"><i class="bi bi-person-plus-fill me-2"></i>Novo Pedido</h1>
                    <p class="text-muted mb-0">Preencha os dados para cadastrar um novo pedido</p>
                </div>
            </div>
        </div>
                                    ...
                                    <div class="input-group mb-2">
                                        <select name="produtos[]" class="form-select" required>
                                            ...
                                            @foreach ($produtos as $produto)
                                                <option value="{{ $produto->id }}">{{ $produto->nome }} ({{ $produtos->where('id', $produto->id)->pluck('medida')->first() ?? 'N/A' }})</option>
                                            @endforeach
                                        </select>
