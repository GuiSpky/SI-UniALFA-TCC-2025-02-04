                ...
                <tbody>
                    @forelse ($cardapios as $cardapio)
                        <tr>
                            ...
                            <td>{{ $cardapio->item }}</td>
                            <td>
                                <i class="bi bi-calendar me-2"></i>{{ \Carbon\Carbon::parse($cardapio->data)->format('d/m/Y') }}
                            </td>
                            <td class="text-end">
                                <a href="{{route('cardapios.edit', $cardapio)}}" class="btn btn-outline-primary btn-sm" title="Editar">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <form action="{{route('cardapios.destroy', $cardapio)}}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-outline-danger btn-sm" type="submit" onclick="return confirm('Deseja realmente apagar este cardápio?')" title="Apagar">
