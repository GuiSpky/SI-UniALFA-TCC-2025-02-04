...
const Sidebar = () => {
      ...
      return (
        <Collapsible key={item.id} open={isExpanded} onOpenChange={() => toggleExpanded(item.id)}>
          <CollapsibleTrigger asChild>
            <Button
              variant="ghost"
              className={`w-full justify-start h-10 px-3 hover:glass-card ${isCollapsed ? 'px-2' : ''
                } ${level > 0 ? 'ml-4' : ''}`}
            >
  ...
  return (
    <div className={`glass-effect border-r border-border transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-50'
      } flex flex-col h-screen animate-fade-in`}>
      <div className="p-4 border-b border-border">
        {isCollapsed ? (
          <div className="flex flex-col items-center space-y-4 mt-2 mb-4">
            <img src="/loguinho.png" alt="Logo GEMA" className=" w-auto" />
            <ThemeToggle />
            <Button
              variant="ghost"
              size="sm"
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <img src="/logo_gema.png" alt="Logo GEMA" className=" w-auto" />
